...
func (s *FlowServiceV2RPC) NodeInsert(
	...
	for _, item := range req.Msg.GetItems() {
		...
		if err := s.ensureFlowWriteAccess(ctx, nodeModel.FlowID); err != nil {
			return nil, err
		}
...
func (s *FlowServiceV2RPC) NodeUpdate(
	...
	for _, item := range req.Msg.GetItems() {
		...
		existing, err := s.ensureNodeWriteAccess(ctx, nodeID)
		if err != nil {
			return nil, err
		}
...
func (s *FlowServiceV2RPC) NodeDelete(
	...
	for _, item := range req.Msg.Items {
		...
		existing, err := s.ensureNodeDeleteAccess(ctx, nodeID)
		if err != nil {
			return nil, err
		}
