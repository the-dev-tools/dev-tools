...
const createNodeExecution = `-- name: CreateNodeExecution :exec
...
// Node Execution
func (q *Queries) CreateNodeExecution(ctx context.Context, arg CreateNodeExecutionParams) error {
	_, err := q.exec(ctx, q.createNodeExecutionStmt, createNodeExecution,
		arg.ID,
		arg.NodeID,
		arg.FlowRunID,
		arg.State,
		arg.Data,
		arg.DataCompressType,
		arg.Error,
	)
	return err
}
...
const getNodeExecutionsByFlowRunID = `-- name: GetNodeExecutionsByFlowRunID :many
SELECT id, node_id, flow_run_id, state, data, data_compress_type, error FROM node_execution WHERE flow_run_id = ? ORDER BY id
`

func (q *Queries) GetNodeExecutionsByFlowRunID(ctx context.Context, flowRunID idwrap.IDWrap) ([]NodeExecution, error) {
	rows, err := q.query(ctx, q.getNodeExecutionsByFlowRunIDStmt, getNodeExecutionsByFlowRunID, flowRunID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, node_id, flow_run_id, state, data, data_compress_type, error FROM node_execution WHERE node_id = ? ORDER BY id DESC
`
