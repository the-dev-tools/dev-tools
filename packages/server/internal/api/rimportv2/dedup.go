...
func (d *Deduplicator) ResolveHTTP(
	...
	// 2. O(1) DB Check (Index Lookup)
	// We no longer filter by is_delta=FALSE because we want to deduplicate Deltas too
...
func (d *Deduplicator) ResolveFile(ctx context.Context, file *mfile.File, logicalPath string) (idwrap.IDWrap, bool, error) {
	...
	// Include WorkspaceID in string to ensure uniqueness across workspaces
