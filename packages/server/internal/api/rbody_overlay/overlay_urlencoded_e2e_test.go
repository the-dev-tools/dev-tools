...
func TestOverlay_URLEncoded_DeltaList_SeedAndClassify(t *testing.T) {
	...
	for _, k := range []string{"a", "b", "c"} {
		if _, err := rpc.BodyUrlEncodedCreate(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedCreateRequest{ExampleId: originEx.ID.Bytes(), Key: k, Enabled: true})); err != nil {
			t.Fatalf("create origin %s: %v", k, err)
		}
	}
	...
	if len(dl1.Msg.Items) != 3 {
		t.Fatalf("expected 3 items, got %d", len(dl1.Msg.Items))
	}
	got := []string{dl1.Msg.Items[0].Key, dl1.Msg.Items[1].Key, dl1.Msg.Items[2].Key}
	want := []string{"a", "b", "c"}
	if got[0] != want[0] || got[1] != want[1] || got[2] != want[2] {
		t.Fatalf("seed order mismatch: want %v, got %v", want, got)
	}
	...
	if len(cr.Msg.BodyId) == 0 {
		t.Fatalf("expected body id for delta create")
	}
	...
	if len(dl2.Msg.Items) != 4 || dl2.Msg.Items[3].Key != "d" {
		t.Fatalf("expected tail d, got len=%d tail=%s", len(dl2.Msg.Items), dl2.Msg.Items[len(dl2.Msg.Items)-1].Key)
	}
}
...
func TestOverlay_URLEncoded_Move_BeforeAfter(t *testing.T) {
	...
	// Move b AFTER d
	var bID []byte
	for _, it := range dl1.Msg.Items {
		if it.Key == "b" {
			bID = it.BodyId
		}
	}
	if _, err := rpc.BodyUrlEncodedDeltaMove(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaMoveRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes(), BodyId: bID, TargetBodyId: dr.Msg.BodyId, Position: resourcesv1.MovePosition_MOVE_POSITION_AFTER})); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 4; i++ {
		if got[i] != want[i] {
			t.Fatalf("after AFTER move: want %v got %v", want, got)
		}
	}
	...
	if _, err := rpc.BodyUrlEncodedDeltaMove(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaMoveRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes(), BodyId: dr.Msg.BodyId, TargetBodyId: aID, Position: resourcesv1.MovePosition_MOVE_POSITION_BEFORE})); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 4; i++ {
		if got2[i] != want2[i] {
			t.Fatalf("after BEFORE move: want %v got %v", want2, got2)
		}
	}
}
...
func TestOverlay_URLEncoded_UpdateResetDelete(t *testing.T) {
	...
	if _, err := rpc.BodyUrlEncodedDeltaUpdate(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaUpdateRequest{BodyId: aID, Value: stringPtr("va"), Enabled: boolPtr(true)})); err != nil {
		t.Fatal(err)
	}
	dl2, _ := rpc.BodyUrlEncodedDeltaList(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaListRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes()}))
	...
	if aItem == nil || aItem.Value != "va" || !aItem.Enabled {
		t.Fatalf("expected a to be MIXED with new value")
	}
	// Reset origin-ref -> ORIGIN
	if _, err := rpc.BodyUrlEncodedDeltaReset(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaResetRequest{BodyId: aID})); err != nil {
		t.Fatal(err)
	}
	dl3, _ := rpc.BodyUrlEncodedDeltaList(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaListRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes()}))
	aItem = nil
	...
	if aItem == nil || aItem.Value == "va" {
		t.Fatalf("expected a reset to origin values")
	}
	// Delete origin-ref -> removed
	if _, err := rpc.BodyUrlEncodedDeltaDelete(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaDeleteRequest{BodyId: aID})); err != nil {
		t.Fatal(err)
	}
	...
	if len(dl4.Msg.Items) != 1 || dl4.Msg.Items[0].Key != "b" {
		t.Fatalf("expected only b after delete, got %d", len(dl4.Msg.Items))
	}
	...
	if _, err := rpc.BodyUrlEncodedDeltaUpdate(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaUpdateRequest{BodyId: dr.Msg.BodyId, Value: stringPtr("vx")})); err != nil {
		t.Fatal(err)
	}
	if _, err := rpc.BodyUrlEncodedDeltaReset(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaResetRequest{BodyId: dr.Msg.BodyId})); err != nil {
		t.Fatal(err)
	}
	...
	var sawX bool
	for _, it := range dl5.Msg.Items {
		if it.BodyId != nil && string(it.BodyId) == string(dr.Msg.BodyId) {
			sawX = true
			if it.Value != "" {
				t.Fatalf("expected cleared value for delta-only reset")
			}
		}
	}
	if !sawX {
		t.Fatalf("expected to find delta-only x in list")
	}
}
