...
func (s *Service) List(ctx context.Context, deltaExampleID, originExampleID idwrap.IDWrap) ([]*bodyv1.BodyUrlEncodedDeltaListItem, error) {
    ...
    if err := s.EnsureSeeded(ctx, deltaExampleID, originExampleID); err != nil { return nil, err }
    ...
    for _, o := range originItems { originByID[o.ID] = o }
    ...
    loadState := func(id idwrap.IDWrap) (stateRow, error) {
        if v, ok := stateCache[id]; ok { return v, nil }
        sr, ok, err := s.ovs.GetState(ctx, deltaExampleID, id)
        if err != nil { return stateRow{}, err }
        if !ok {
    ...
    coalesce := func(ns sql.NullString, def string) string { if ns.Valid { return ns.String }; return def }
    coalesceB := func(nb sql.NullBool, def bool) bool { if nb.Valid { return nb.Bool }; return def }
...
func (s *Service) CreateDelta(ctx context.Context, deltaExampleID idwrap.IDWrap) (idwrap.IDWrap, error) {
    id := idwrap.NewNow()
    if err := s.ovs.InsertDelta(ctx, deltaExampleID, id, "", "", "", true); err != nil { return idwrap.IDWrap{}, err }
    // tail rank
...
func (s *Service) Update(ctx context.Context, deltaExampleID idwrap.IDWrap, bodyID idwrap.IDWrap, key, val, desc *string, enabled *bool) error {
    ...
    if ok, _ := s.ovs.ExistsDelta(ctx, deltaExampleID, bodyID); ok {
        ...
        if err != nil { return err }
        if !found { return errors.New("delta row disappeared during update") }
        if key != nil { ck = *key }
        if val != nil { cv = *val }
        if desc != nil { cd = *desc }
        if enabled != nil { ce = *enabled }
...
func (s *Service) Move(ctx context.Context, deltaExampleID, originExampleID idwrap.IDWrap, bodyID, targetID idwrap.IDWrap, after bool) error {
    ...
    if err != nil { return err }
    if len(ord) == 0 {
        if err := s.EnsureSeeded(ctx, deltaExampleID, originExampleID); err != nil { return err }
        ord, err = s.readOrder(ctx, deltaExampleID)
        if err != nil { return err }
    }
    // ensure moving item exists; if not, insert with tail rank so we can then move
    idxBy := map[string]int{}
    for i, r := range ord { idxBy[r.RefID.String()] = i }
    ...
    if after {
        ...
        if tIdx+1 < len(ord) { rightRank = ord[tIdx+1].Rank } else { rightRank = "" }
    } else {
    ...
    rk := refKindOrigin
    if ok, _ := s.ovs.ExistsDelta(ctx, deltaExampleID, bodyID); ok { rk = refKindDelta }
    // upsert
...
func (s *Service) Undelete(ctx context.Context, deltaExampleID idwrap.IDWrap, bodyID idwrap.IDWrap) error {
    // unsuppress
    if err := s.ovs.UnsuppressState(ctx, deltaExampleID, bodyID); err != nil { return err }
    // append order entry if not exists
    if ok, _ := s.ovs.ExistsOrderRow(ctx, deltaExampleID, refKindOrigin, bodyID); ok { return nil }
    last, ok, err := s.ovs.LastOrderRank(ctx, deltaExampleID)
    if err != nil { return err }
    var newRank string
    if ok { newRank = orank.Between(last, "") } else { newRank = orank.First() }
...
func strPtrOrNil(p *string) interface{} { if p == nil { return nil }; return *p }
func boolPtrOrNil(p *bool) interface{} { if p == nil { return nil }; return *p }
...
func (s *Service) ResolveExampleForBodyID(ctx context.Context, bodyID idwrap.IDWrap) (idwrap.IDWrap, bool, error) {
    if ex, ok, err := s.ovs.ResolveExampleByDeltaID(ctx, bodyID); err != nil { return idwrap.IDWrap{}, false, err } else if ok { return ex, true, nil }
    if ex, ok, err := s.ovs.ResolveExampleByOrderRefID(ctx, bodyID); err != nil { return idwrap.IDWrap{}, false, err } else if ok { return ex, true, nil }
