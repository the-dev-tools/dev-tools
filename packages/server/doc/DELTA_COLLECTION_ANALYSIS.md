...
**Verdict:** **Correct.**
We do _not_ need to send an explicit `UNSET` signal in a Collection response. Omitting the field correctly represents "no override exists".
...
3.  If it sees `KIND_UNSET`, it _explicitly sets the field to undefined_ in the local store.
