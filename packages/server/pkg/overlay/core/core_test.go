...
type entry struct { kind int8; id idwrap.IDWrap; rank string; rev int64 }
...
func (m *memOrder) Count(ctx context.Context, ex idwrap.IDWrap) (int64, error) { return int64(len(m.rows[exKeyStr(ex)])), nil }
func (m *memOrder) SelectAsc(ctx context.Context, ex idwrap.IDWrap) ([]core.OrderRow, error) {
    ...
    for i := 0; i < len(arr); i++ { for j := i+1; j < len(arr); j++ { if arr[j].rank < arr[i].rank { arr[i], arr[j] = arr[j], arr[i] } } }
    out := make([]core.OrderRow, 0, len(arr))
    for _, e := range arr { out = append(out, core.OrderRow{ RefKind: e.kind, RefID: e.id, Rank: e.rank }) }
    return out, nil
}
func (m *memOrder) LastRank(ctx context.Context, ex idwrap.IDWrap) (string, bool, error) {
    ...
    // find max
    max := rows[0].rank
    for _, e := range rows { if e.rank > max { max = e.rank } }
    return max, true, nil
}
func (m *memOrder) MaxRevision(ctx context.Context, ex idwrap.IDWrap) (int64, error) {
    rows := m.rows[exKeyStr(ex)]
    var max int64
    for _, e := range rows { if e.rev > max { max = e.rev } }
    return max, nil
}
func (m *memOrder) InsertIgnore(ctx context.Context, ex idwrap.IDWrap, refKind int8, refID idwrap.IDWrap, rank string, revision int64) error {
    k := exKeyStr(ex)
    for _, e := range m.rows[k] { if e.kind == refKind && e.id.Compare(refID) == 0 { return nil } }
...
func (m *memOrder) Upsert(ctx context.Context, ex idwrap.IDWrap, refKind int8, refID idwrap.IDWrap, rank string, revision int64) error {
    ...
    for i, e := range m.rows[k] {
        if e.kind == refKind && e.id.Compare(refID) == 0 { m.rows[k][i].rank = rank; m.rows[k][i].rev = revision; return nil }
    }
    m.rows[k] = append(m.rows[k], entry{ kind: refKind, id: refID, rank: rank, rev: revision })
    return nil
}
func (m *memOrder) DeleteByRef(ctx context.Context, ex idwrap.IDWrap, refID idwrap.IDWrap) error {
    ...
    for _, e := range m.rows[k] { if e.id.Compare(refID) != 0 { out = append(out, e) } }
    m.rows[k] = out
    return nil
}
func (m *memOrder) Exists(ctx context.Context, ex idwrap.IDWrap, refKind int8, refID idwrap.IDWrap) (bool, error) {
    k := exKeyStr(ex)
    for _, e := range m.rows[k] { if e.kind == refKind && e.id.Compare(refID) == 0 { return true, nil } }
    return false, nil
}

type memState struct { m map[string]map[string]core.StateRow }
...
func (s *memState) Upsert(ctx context.Context, ex, origin idwrap.IDWrap, suppressed bool, k, val, desc *string, enabled *bool) error {
    ...
    if !ok { ek = map[string]core.StateRow{}; s.m[exKeyStr(ex)] = ek }
    row := ek[idKeyStr(origin)]
    row.Suppressed = suppressed
    if k != nil { row.Key = k }
    if val != nil { row.Val = val }
    if desc != nil { row.Desc = desc }
    if enabled != nil { row.Enabled = enabled }
    ek[idKeyStr(origin)] = row
    return nil
}
func (s *memState) Get(ctx context.Context, ex, origin idwrap.IDWrap) (core.StateRow, bool, error) {
    ek := s.m[exKeyStr(ex)]; if ek == nil { return core.StateRow{}, false, nil }
    row, ok := ek[idKeyStr(origin)]; if !ok { return core.StateRow{}, false, nil }
    return row, true, nil
}
func (s *memState) ClearOverrides(ctx context.Context, ex, origin idwrap.IDWrap) error {
    ek := s.m[exKeyStr(ex)]; if ek == nil { return nil }
    row := ek[idKeyStr(origin)]
...
func (s *memState) Suppress(ctx context.Context, ex, origin idwrap.IDWrap) error {
    ek, ok := s.m[exKeyStr(ex)]; if !ok { ek = map[string]core.StateRow{}; s.m[exKeyStr(ex)] = ek }
    row := ek[idKeyStr(origin)]; row.Suppressed = true; ek[idKeyStr(origin)] = row; return nil
}
func (s *memState) Unsuppress(ctx context.Context, ex, origin idwrap.IDWrap) error {
    ek := s.m[exKeyStr(ex)]; if ek == nil { return nil }
    row := ek[idKeyStr(origin)]; row.Suppressed = false; ek[idKeyStr(origin)] = row; return nil
}

type memDelta struct { m map[string]map[string]drow }
type drow struct { k, v, d string; e bool }
...
func (d *memDelta) Insert(ctx context.Context, ex, id idwrap.IDWrap, key, value, desc string, enabled bool) error {
    ek := d.m[exKeyStr(ex)]; if ek == nil { ek = map[string]drow{}; d.m[exKeyStr(ex)] = ek }
...
func (d *memDelta) Get(ctx context.Context, ex, id idwrap.IDWrap) (string, string, string, bool, bool, error) {
    ek := d.m[exKeyStr(ex)]; if ek == nil { return "", "", "", false, false, nil }
    row, ok := ek[idKeyStr(id)]; if !ok { return "", "", "", false, false, nil }
    return row.k, row.v, row.d, row.e, true, nil
}
func (d *memDelta) Exists(ctx context.Context, ex, id idwrap.IDWrap) (bool, error) {
    ek := d.m[exKeyStr(ex)]; if ek == nil { return false, nil }
    _, ok := ek[idKeyStr(id)]; return ok, nil
}
func (d *memDelta) Delete(ctx context.Context, ex, id idwrap.IDWrap) error {
    ek := d.m[exKeyStr(ex)]; if ek == nil { return nil }
    delete(ek, idKeyStr(id)); return nil
}
...
type testItem struct { id idwrap.IDWrap; vals core.Values; origin *core.Values; src deltav1.SourceKind }
...
func TestCore_List_ClassifyOriginMixedDelta(t *testing.T) {
    ...
    ov := "va2"; _ = state.Upsert(ctx, ex, a, false, nil, &ov, nil, nil)
    same := "kb"; _ = state.Upsert(ctx, ex, b, false, &same, nil, nil, nil)
    // Delta-only
    _ = delta.Insert(ctx, ex, d, "kd", "vd", "dd", true)

    build := func(m core.Merged) any {
        var o *core.Values
        if m.Origin != nil { v := *m.Origin; o = &v }
    ...
    for _, it := range itemsAny { items = append(items, it.(testItem)) }
    if len(items) != 3 { t.Fatalf("expected 3 merged items, got %d", len(items)) }
    // A should be MIXED
...
func TestCore_Move_Order(t *testing.T) {
    ctx := context.Background()
    ex := idwrap.NewNow()
    a := idwrap.NewNow(); b := idwrap.NewNow(); d := idwrap.NewNow()
    order := newMemOrder()
    // seed order A,B,D
    ...
    if err := core.Move(ctx, order, ex, a, b, true); err != nil { t.Fatal(err) }
    ...
    if err := core.Move(ctx, order, ex, d, b, false); err != nil { t.Fatal(err) }
    ord, _ = order.SelectAsc(ctx, ex)
...
func TestCore_Reset_Delete_Undelete(t *testing.T) {
    ctx := context.Background()
    ex := idwrap.NewNow(); oex := idwrap.NewNow()
    a := idwrap.NewNow(); d := idwrap.NewNow()
    order := newMemOrder(); state := newMemState(); delta := newMemDelta()
    ...
    // set override for A
    nv := "va2"; _ = state.Upsert(ctx, ex, a, false, nil, &nv, nil, nil)
    // set delta values for D
    _ = delta.Insert(ctx, ex, d, "kd", "vd", "dd", false)
    // Reset A and D
    if err := core.Reset(ctx, state, delta, ex, a); err != nil { t.Fatal(err) }
    if err := core.Reset(ctx, state, delta, ex, d); err != nil { t.Fatal(err) }
    ...
    if !foundA || !foundD { t.Fatalf("unexpected merged after reset: A:%v D:%v", foundA, foundD) }
    // Delete A (origin) => suppressed
    if err := core.Delete(ctx, order, state, delta, ex, a); err != nil { t.Fatal(err) }
    itemsAny, err = core.List(ctx, order, state, delta, func(context.Context, idwrap.IDWrap) ([]struct{}, error) { return nil, nil }, func(struct{}) core.Values { return core.Values{} }, originValsFrom(origins), build, ex, oex)
    if err != nil { t.Fatal(err) }
    for _, it := range itemsAny { if it.(core.Merged).ID.Compare(a) == 0 { t.Fatalf("expected A suppressed and removed from list") } }
    // Undelete A => appended at tail
    if err := core.Undelete(ctx, order, state, ex, a); err != nil { t.Fatal(err) }
    ord, _ := order.SelectAsc(ctx, ex)
    if ord[len(ord)-1].RefID.Compare(a) != 0 { t.Fatalf("expected A appended at tail on undelete") }
}

func TestCore_Delete_DeltaOnly_NoSuppress(t *testing.T) {
    ctx := context.Background()
    ex := idwrap.NewNow(); d := idwrap.NewNow()
    order := newMemOrder(); state := newMemState(); delta := newMemDelta()
    ...
    // delete
    if err := core.Delete(ctx, order, state, delta, ex, d); err != nil { t.Fatal(err) }
    // ensure removed from order
    ord, _ := order.SelectAsc(ctx, ex)
    if len(ord) != 0 { t.Fatalf("expected empty order after delta delete, got %v", ord) }
    // ensure no state row created
    if _, ok, _ := state.Get(ctx, ex, d); ok { t.Fatalf("expected no state row for delta-only delete") }
}
