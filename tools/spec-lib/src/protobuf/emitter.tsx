...
import { Projects, unauthenticatedOperations, useProject } from '../core/index.js';
...
export const $onEmit = async (context: EmitContext<(typeof EmitterOptions)['Encoded']>) => {
  ...
  // Generate Go file with unauthenticated endpoints
  await writeOutput(
    program,
    <Output printWidth={120} program={program}>
      <Projects>{(_) => <UnauthenticatedEndpointsGo namespace={_.namespace} />}</Projects>
    </Output>,
    join(emitterOutputDir, 'buf/go/authutil'),
  );
};
...
// =============================================================================
// Go Unauthenticated Endpoints Generator
// =============================================================================

interface UnauthenticatedEndpointsGoProps {
  namespace: Namespace;
}

const UnauthenticatedEndpointsGo = ({ namespace }: UnauthenticatedEndpointsGoProps) => {
  const { program } = useTsp();

  // Collect all public operations from this namespace and nested namespaces
  const collectPublicOps = (ns: Namespace): { service: string; operation: string }[] => {
    const ops: { service: string; operation: string }[] = [];

    // Get operations from interfaces (services)
    ns.interfaces.forEach((iface) => {
      iface.operations.forEach((op) => {
        if (unauthenticatedOperations(program).has(op)) {
          ops.push({ service: iface.name, operation: op.name });
        }
      });
    });

    // Get operations from namespace directly (default service)
    const defaultService = `${ns.name}Service`;
    ns.operations.forEach((op) => {
      if (unauthenticatedOperations(program).has(op)) {
        ops.push({ service: defaultService, operation: op.name });
      }
    });

    // Recurse into nested namespaces
    ns.namespaces.forEach((nested) => {
      ops.push(...collectPublicOps(nested));
    });

    return ops;
  };

  const publicOps = collectPublicOps(namespace);

  // If no public operations, don't generate the file
  if (publicOps.length === 0) return null;

  // Group by service
  const byService = pipe(
    publicOps,
    Array.groupBy((_) => _.service),
    Record.map((_) => _.map((op) => op.operation)),
  );

  return (
    <SourceFile filetype='string' path='unauthenticated.go'>
      {`// Code generated by @the-dev-tools/spec-lib. DO NOT EDIT.
// This file contains operations marked with @unauthenticated decorator.

package authutil

import "strings"

// UnauthenticatedEndpoints contains operation names that don't require authentication.
// Generated from operations decorated with @unauthenticated in TypeSpec.
var UnauthenticatedEndpoints = map[string]struct{}{
${Object.entries(byService)
  .flatMap(([_service, ops]) => ops.map((op) => `\t"${op}": {},`))
  .join('\n')}
}

// IsUnauthenticated checks if a procedure name ends with an unauthenticated endpoint.
// Example: IsUnauthenticated("/api.auth.v1.AuthService/SignIn") returns true
func IsUnauthenticated(procedure string) bool {
\tfor name := range UnauthenticatedEndpoints {
\t\tif strings.HasSuffix(procedure, "/"+name) {
\t\t\treturn true
\t\t}
\t}
\treturn false
