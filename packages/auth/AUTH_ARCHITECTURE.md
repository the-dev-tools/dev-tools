...
6. **Desktop/local mode unchanged.** Uses dummy user, no external auth needed.
...
| `GetSSOUrl` | No | Get SSO authorization URL (not yet implemented) |
| `HandleSSOCallback` | No | Handle SSO callback (not yet implemented) |
...
- `local` (default): Desktop/CLI mode. Uses dummy user (`00000000000000000000000000`). No external auth.
- `betterauth`: SaaS mode. Validates JWTs via JWKS public keys. Auto-provisions users.
...
-- account: Linked auth providers (credential, Google, GitHub, etc.)
...
| `MICROSOFT_CLIENT_ID` | No | — | Microsoft OAuth credentials |
