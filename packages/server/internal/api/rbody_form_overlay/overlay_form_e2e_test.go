...
func TestBodyFormDelta_Overlay_Flow(t *testing.T) {
	...
	for _, k := range []string{"A", "B"} {
		...
		if err != nil {
			t.Fatalf("create origin form %s: %v", k, err)
		}
	}
	...
	if len(l1.Msg.Items) != 2 {
		t.Fatalf("expected 2 seed items, got %d", len(l1.Msg.Items))
	}
	...
	if len(aID) == 0 {
		t.Fatalf("failed to find origin A in list")
	}
	if _, err := rpc.BodyFormDeltaUpdate(authed, connect.NewRequest(&bodyv1.BodyFormDeltaUpdateRequest{BodyId: aID, Value: stringPtr("VA"), Enabled: boolPtr(true)})); err != nil {
		t.Fatal(err)
	}
	...
	// Reset mixed
	if _, err := rpc.BodyFormDeltaReset(authed, connect.NewRequest(&bodyv1.BodyFormDeltaResetRequest{BodyId: aMixed.BodyId})); err != nil {
		t.Fatal(err)
	}
	...
	// Delete delta-only C
	if _, err := rpc.BodyFormDeltaDelete(authed, connect.NewRequest(&bodyv1.BodyFormDeltaDeleteRequest{BodyId: cr.Msg.BodyId})); err != nil {
		t.Fatal(err)
	}
	...
	var bID []byte
	for _, it := range l4.Msg.Items {
		if it.Origin != nil && it.Origin.Key == "B" {
			bID = it.BodyId
		}
	}
	if _, err := rpc.BodyFormDeltaMove(authed, connect.NewRequest(&bodyv1.BodyFormDeltaMoveRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes(), BodyId: aAfter.BodyId, TargetBodyId: bID, Position: resourcesv1.MovePosition_MOVE_POSITION_AFTER})); err != nil {
		t.Fatal(err)
	}
	...
	keys := func(items []*bodyv1.BodyFormDeltaListItem) []string {
		out := []string{}
	...
	if len(got) != 2 || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("order after move mismatch: got %v want %v", got, want)
	}
}
