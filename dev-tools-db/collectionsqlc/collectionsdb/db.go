...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getByOwnerIDStmt, err = db.PrepareContext(ctx, getByOwnerID); err != nil {
		return nil, fmt.Errorf("error preparing query GetByOwnerID: %w", err)
	}
	...
	if q.getOwnerIDStmt, err = db.PrepareContext(ctx, getOwnerID); err != nil {
		return nil, fmt.Errorf("error preparing query GetOwnerID: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getByOwnerIDStmt != nil {
		if cerr := q.getByOwnerIDStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getByOwnerIDStmt: %w", cerr)
		}
	}
	...
	if q.getOwnerIDStmt != nil {
		if cerr := q.getOwnerIDStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getOwnerIDStmt: %w", cerr)
		}
	}
	if q.updateStmt != nil {
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getStmt:                    q.getStmt,
		getByOwnerIDStmt:           q.getByOwnerIDStmt,
		getByPlatformIDandTypeStmt: q.getByPlatformIDandTypeStmt,
		getOwnerIDStmt:             q.getOwnerIDStmt,
