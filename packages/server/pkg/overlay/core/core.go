...
func List[O any](ctx context.Context,
	...
	for _, r := range ord {
		if r.RefKind == RefKindOrigin {
			originIDs = append(originIDs, r.RefID)
		}
	}
	...
	for _, r := range ord {
		...
		case RefKindOrigin:
			...
			mixed := false
			if st.Key != nil && *st.Key != orig.Key {
				mixed = true
			}
			if st.Val != nil && *st.Val != orig.Value {
				mixed = true
			}
			if st.Desc != nil && *st.Desc != orig.Description {
				mixed = true
			}
			if st.Enabled != nil && *st.Enabled != orig.Enabled {
				mixed = true
			}
			if !mixed {
...
func CreateDelta(ctx context.Context, order OrderStore, delta DeltaStore, deltaEx idwrap.IDWrap) (idwrap.IDWrap, error) {
	id := idwrap.NewNow()
	if err := delta.Insert(ctx, deltaEx, id, "", "", "", true); err != nil {
	...
	if err := order.Upsert(ctx, deltaEx, RefKindDelta, id, newRank, max+1); err != nil {
		return idwrap.IDWrap{}, err
	}
	return id, nil
}
...
func Update(ctx context.Context, state StateStore, delta DeltaStore, deltaEx idwrap.IDWrap, id idwrap.IDWrap, vals *Values) error {
	if ok, _ := delta.Exists(ctx, deltaEx, id); ok {
		...
		if !found {
			return errors.New("delta disappeared")
		}
		if vals != nil {
			if vals.Key != "" {
				k = vals.Key
...
func Move(ctx context.Context, order OrderStore, deltaEx, id, target idwrap.IDWrap, after bool) error {
	...
	for i, r := range ord {
		...
		kindBy[r.RefID.String()] = r.RefKind
	}
	...
	if after {
		left = ord[tIdx].Rank
		if tIdx+1 < len(ord) {
			right = ord[tIdx+1].Rank
		}
	} else {
		right = ord[tIdx].Rank
		if tIdx-1 >= 0 {
			left = ord[tIdx-1].Rank
		}
	}
...
func Delete(ctx context.Context, order OrderStore, state StateStore, delta DeltaStore, deltaEx, id idwrap.IDWrap) error {
	...
	if ok, _ := delta.Exists(ctx, deltaEx, id); ok {
		if err := delta.Delete(ctx, deltaEx, id); err != nil {
			return err
		}
		if err := order.DeleteByRef(ctx, deltaEx, id); err != nil {
			return err
		}
		return nil
	}
...
func Undelete(ctx context.Context, order OrderStore, state StateStore, deltaEx, id idwrap.IDWrap) error {
	if err := state.Unsuppress(ctx, deltaEx, id); err != nil {
		return err
	}
	if ok, _ := order.Exists(ctx, deltaEx, RefKindOrigin, id); ok {
		return nil
	}
	last, ok, err := order.LastRank(ctx, deltaEx)
	if err != nil {
...
func ResolveExampleForBodyID(ctx context.Context, resolveDelta ResolveDeltaFn, resolveOrder ResolveOrderFn, id idwrap.IDWrap) (idwrap.IDWrap, bool, error) {
	if ex, ok, err := resolveDelta(ctx, id); err != nil {
		return idwrap.IDWrap{}, false, err
	} else if ok {
		return ex, true, nil
	}
	if ex, ok, err := resolveOrder(ctx, id); err != nil {
		return idwrap.IDWrap{}, false, err
	} else if ok {
		return ex, true, nil
	}
