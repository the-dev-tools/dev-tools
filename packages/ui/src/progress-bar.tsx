    ...
    {({ percentage, valueText }) => (
      <>
        <div className={tw`flex justify-between gap-2`}>
          ...
          <span className={tw`text-sm text-neutral-600`}>{valueText}</span>
        </div>

        <div
          className={tw`
            relative h-2 max-w-full overflow-hidden rounded-full bg-neutral-300 outline-1 -outline-offset-1
            outline-transparent
          `}
        >
          <div className={tw`absolute top-0 h-full rounded-full bg-violet-600`} style={{ width: `${percentage}%` }} />
        </div>
      </>
    )}
  </RAC.ProgressBar>
