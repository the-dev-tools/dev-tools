...
func TestUrlEncoded_DeltaOrderingAndMove_E2E(t *testing.T) {
	...
	for _, k := range []string{"11", "22", "33"} {
		...
		if _, err := rpc.BodyUrlEncodedCreate(authed, req); err != nil {
	...
	if len(list1.Msg.Items) != 3 {
		t.Fatalf("expected 3 delta items (proxies), got %d", len(list1.Msg.Items))
	}
	...
	if len(list2.Msg.Items) != 4 {
		t.Fatalf("expected 4 delta items, got %d", len(list2.Msg.Items))
	}
	// 44 should be last
	if list2.Msg.Items[3].Key != "44" {
		t.Fatalf("expected tail to be 44, got %s", list2.Msg.Items[3].Key)
	}
	...
	if list3.Msg.Items[0].Key != "44" {
		t.Fatalf("expected 44 to be first after move, got %s", list3.Msg.Items[0].Key)
	}
	...
	if len(listOrigin.Msg.Items) != 3 {
		t.Fatalf("expected origin count 3, got %d", len(listOrigin.Msg.Items))
	}
}
...
func TestUrlEncoded_DeltaMove_After_E2E(t *testing.T) {
	...
	if len(list1.Msg.Items) != 3 {
		t.Fatalf("expected 3 delta items, got %d", len(list1.Msg.Items))
	}
	...
	if bytes.Equal(last.BodyId, bID) {
		return
	}
...
func TestUrlEncoded_DeltaUpdate_Rules(t *testing.T) {
	...
	if len(dl.Msg.Items) != 1 {
		t.Fatalf("expected 1 proxy, got %d", len(dl.Msg.Items))
	}
	proxyID := dl.Msg.Items[0].BodyId

	// Update proxy in place: make it mixed
	if _, err := rpc.BodyUrlEncodedDeltaUpdate(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaUpdateRequest{BodyId: proxyID, Value: stringPtr("v")})); err != nil {
		t.Fatal(err)
	}
	...
	if *dl2.Msg.Items[0].Source != deltav1.SourceKind_SOURCE_KIND_MIXED {
		t.Fatalf("expected mixed after update")
	}
...
func TestUrlEncoded_Delta_ProxiesAppendAfterDeltaOnlyWhenNoAnchors(t *testing.T) {
	...
	// Seed proxies first
	if _, err := rpc.BodyUrlEncodedDeltaList(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaListRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes()})); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := rpc.BodyUrlEncodedDeltaUpdate(authed, connect.NewRequest(&bodyv1.BodyUrlEncodedDeltaUpdateRequest{BodyId: dy.Msg.BodyId, Key: stringPtr("y"), Enabled: boolPtr(true)})); err != nil {
		t.Fatal(err)
	}
