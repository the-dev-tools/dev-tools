...
func TestHeaderDelta_Smoke(t *testing.T) {
	...
	if len(l1.Msg.Items) != 2 {
		t.Fatalf("expected 2 items, got %d", len(l1.Msg.Items))
	}
	...
	if len(cr.Msg.HeaderId) == 0 {
		t.Fatalf("expected header id for delta create")
	}
	...
	if len(aID) == 0 {
		t.Fatalf("failed to find origin A in delta list")
	}
	if _, err := rpc.HeaderDeltaUpdate(authed, connect.NewRequest(&requestv1.HeaderDeltaUpdateRequest{HeaderId: aID, Value: stringPtr("VA"), Enabled: boolPtr(true)})); err != nil {
		t.Fatal(err)
	}
	...
	if aMixed == nil || aMixed.Value != "VA" || !aMixed.Enabled {
		t.Fatalf("expected A mixed updated")
	}

	// Reset mixed by its delta id
	if _, err := rpc.HeaderDeltaReset(authed, connect.NewRequest(&requestv1.HeaderDeltaResetRequest{HeaderId: aMixed.HeaderId})); err != nil {
		t.Fatal(err)
	}
	...
	for _, it := range l3.Msg.Items {
		if it.Origin != nil && it.Origin.Key == "A" {
			aAfter = it
			break
		}
	}
	if aAfter == nil || aAfter.Value == "VA" {
		t.Fatalf("expected A reset to origin value")
	}

	// Delete delta-only C
	if _, err := rpc.HeaderDeltaDelete(authed, connect.NewRequest(&requestv1.HeaderDeltaDeleteRequest{HeaderId: cr.Msg.HeaderId})); err != nil {
		t.Fatal(err)
	}
	...
	if len(l4.Msg.Items) != 2 {
		t.Fatalf("expected 2 items after delete, got %d", len(l4.Msg.Items))
	}
}
...
func TestQueryDelta_Smoke(t *testing.T) {
	...
	if len(cr.Msg.QueryId) == 0 {
		t.Fatalf("expected query id for delta create")
	}

	// Update origin-ref 'q1' (mixed)
	var q1ID []byte
	for _, it := range l1.Msg.Items {
		if it.Origin != nil && it.Origin.Key == "q1" {
			q1ID = it.QueryId
			break
		}
	}
	if len(q1ID) == 0 {
		t.Fatalf("failed to find origin q1 in delta list")
	}
	if _, err := rpc.QueryDeltaUpdate(authed, connect.NewRequest(&requestv1.QueryDeltaUpdateRequest{QueryId: q1ID, Value: stringPtr("v1"), Enabled: boolPtr(true)})); err != nil {
		t.Fatal(err)
	}
	...
	if q1Mixed == nil || q1Mixed.Value != "v1" || !q1Mixed.Enabled {
		t.Fatalf("expected q1 mixed updated")
	}

	// Reset mixed by its delta id
	if _, err := rpc.QueryDeltaReset(authed, connect.NewRequest(&requestv1.QueryDeltaResetRequest{QueryId: q1Mixed.QueryId})); err != nil {
		t.Fatal(err)
	}
	...
	if q1After == nil || q1After.Value == "v1" {
		t.Fatalf("expected q1 reset to origin value")
	}

	// Delete delta-only q3
	if _, err := rpc.QueryDeltaDelete(authed, connect.NewRequest(&requestv1.QueryDeltaDeleteRequest{QueryId: cr.Msg.QueryId})); err != nil {
		t.Fatal(err)
	}
...
func TestHeaderDelta_Move(t *testing.T) {
	...
	for _, it := range l1.Msg.Items {
		...
		if it.Origin != nil && it.Origin.Key == "B" {
			bID = it.HeaderId
		}
	}
	if len(aID) == 0 || len(bID) == 0 {
		t.Fatalf("failed to resolve A/B ids from initial list")
	}
...
func TestQueryDelta_Move(t *testing.T) {
	...
	for _, it := range l1.Msg.Items {
		...
		if it.Origin != nil && it.Origin.Key == "q2" {
			q2ID = it.QueryId
		}
	}
	if len(q1ID) == 0 || len(q2ID) == 0 {
		t.Fatalf("failed to resolve q1/q2 ids")
	}
	...
	// Verify [q2, q1, q3]
	l2, _ := rpc.QueryDeltaList(authed, connect.NewRequest(&requestv1.QueryDeltaListRequest{ExampleId: deltaEx.ID.Bytes(), OriginId: originEx.ID.Bytes()}))
	keys := func(items []*requestv1.QueryDeltaListItem) []string {
		out := []string{}
		for _, it := range items {
			if it.Origin != nil {
				out = append(out, it.Origin.Key)
