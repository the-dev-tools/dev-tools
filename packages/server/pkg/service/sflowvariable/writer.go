...
func (w *Writer) DeleteFlowVariable(ctx context.Context, id idwrap.IDWrap) error {
	err := w.queries.DeleteFlowVariable(ctx, id)
	return tgeneric.ReplaceRootWithSub(sql.ErrNoRows, ErrNoFlowVariableFound, err)
}

// UpdateFlowVariableOrder updates the display_order for a single flow variable
func (w *Writer) UpdateFlowVariableOrder(ctx context.Context, id idwrap.IDWrap, order float64) error {
...
func (w *Writer) MoveFlowVariableAfter(ctx context.Context, variableID, targetVariableID idwrap.IDWrap) error {
...
func (w *Writer) MoveFlowVariableBefore(ctx context.Context, variableID, targetVariableID idwrap.IDWrap) error {
	// Validate the move operation
	if variableID.Compare(targetVariableID) == 0 {
		return errors.New("cannot move flow variable relative to itself")
	}
...
func (w *Writer) ReorderFlowVariables(ctx context.Context, orderedIDs []idwrap.IDWrap) error {
...
func (w *Writer) checkFlowBoundaries(ctx context.Context, variableID, targetVariableID idwrap.IDWrap) error {
	sourceVariable, err := w.reader.GetFlowVariable(ctx, variableID)
	if err != nil {
		return fmt.Errorf("failed to get source flow variable: %w", err)
	}
