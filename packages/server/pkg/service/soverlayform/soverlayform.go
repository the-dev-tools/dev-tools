...
type Service struct {
    ...
    stmtCountOrder, stmtSelectOrderAsc, stmtLastOrderRank, stmtMaxOrderRevision *sql.Stmt
    stmtInsertOrderIgnore, stmtUpsertOrder, stmtDeleteOrderByRef, stmtExistsOrderRow *sql.Stmt

    stmtInsertDelta, stmtUpdateDelta, stmtGetDelta, stmtExistsDelta, stmtDeleteDelta *sql.Stmt

    stmtUpsertState, stmtGetState, stmtClearStateOverrides, stmtSuppressState, stmtUnsuppressState *sql.Stmt

    stmtResolveByDeltaID, stmtResolveByOrderRefID *sql.Stmt
}

func New(db *sql.DB) (*Service, error) {
    s := &Service{db: db}
    var err error
    if s.stmtCountOrder, err = db.Prepare(`SELECT COUNT(*) FROM delta_form_order WHERE example_id = ?`); err != nil { return nil, err }
    if s.stmtSelectOrderAsc, err = db.Prepare(`SELECT ref_kind, ref_id, rank, revision FROM delta_form_order WHERE example_id = ? ORDER BY rank ASC`); err != nil { return nil, err }
    if s.stmtLastOrderRank, err = db.Prepare(`SELECT rank FROM delta_form_order WHERE example_id = ? ORDER BY rank DESC LIMIT 1`); err != nil { return nil, err }
    if s.stmtMaxOrderRevision, err = db.Prepare(`SELECT MAX(revision) FROM delta_form_order WHERE example_id = ?`); err != nil { return nil, err }
    if s.stmtInsertOrderIgnore, err = db.Prepare(`INSERT OR IGNORE INTO delta_form_order(example_id, ref_kind, ref_id, rank, revision) VALUES (?,?,?,?,?)`); err != nil { return nil, err }
    if s.stmtUpsertOrder, err = db.Prepare(`INSERT INTO delta_form_order(example_id, ref_kind, ref_id, rank, revision) VALUES (?,?,?,?,?) ON CONFLICT(example_id, ref_kind, ref_id) DO UPDATE SET rank=excluded.rank, revision=excluded.revision`); err != nil { return nil, err }
    if s.stmtDeleteOrderByRef, err = db.Prepare(`DELETE FROM delta_form_order WHERE example_id = ? AND ref_id = ?`); err != nil { return nil, err }
    if s.stmtExistsOrderRow, err = db.Prepare(`SELECT 1 FROM delta_form_order WHERE example_id = ? AND ref_kind = ? AND ref_id = ?`); err != nil { return nil, err }

    if s.stmtInsertDelta, err = db.Prepare(`INSERT INTO delta_form_delta(example_id, id, body_key, value, description, enabled) VALUES (?,?,?,?,?, ?)`); err != nil { return nil, err }
    if s.stmtUpdateDelta, err = db.Prepare(`UPDATE delta_form_delta SET body_key = ?, value = ?, description = ?, enabled = ?, updated_at = unixepoch() WHERE example_id = ? AND id = ?`); err != nil { return nil, err }
    if s.stmtGetDelta, err = db.Prepare(`SELECT body_key, value, description, enabled FROM delta_form_delta WHERE example_id = ? AND id = ?`); err != nil { return nil, err }
    if s.stmtExistsDelta, err = db.Prepare(`SELECT 1 FROM delta_form_delta WHERE example_id = ? AND id = ?`); err != nil { return nil, err }
    if s.stmtDeleteDelta, err = db.Prepare(`DELETE FROM delta_form_delta WHERE example_id = ? AND id = ?`); err != nil { return nil, err }

    if s.stmtUpsertState, err = db.Prepare(`INSERT INTO delta_form_state(example_id, origin_id, suppressed, body_key, value, description, enabled, updated_at) VALUES (?,?,?,?,?,?,?, unixepoch()) ON CONFLICT(example_id, origin_id) DO UPDATE SET suppressed=excluded.suppressed, body_key=COALESCE(excluded.body_key, delta_form_state.body_key), value=COALESCE(excluded.value, delta_form_state.value), description=COALESCE(excluded.description, delta_form_state.description), enabled=COALESCE(excluded.enabled, delta_form_state.enabled), updated_at=unixepoch()`); err != nil { return nil, err }
    if s.stmtGetState, err = db.Prepare(`SELECT suppressed, body_key, value, description, enabled FROM delta_form_state WHERE example_id = ? AND origin_id = ?`); err != nil { return nil, err }
    if s.stmtClearStateOverrides, err = db.Prepare(`UPDATE delta_form_state SET body_key=NULL, value=NULL, description=NULL, enabled=NULL, updated_at = unixepoch() WHERE example_id = ? AND origin_id = ?`); err != nil { return nil, err }
    if s.stmtSuppressState, err = db.Prepare(`INSERT INTO delta_form_state(example_id, origin_id, suppressed, updated_at) VALUES (?,?, TRUE, unixepoch()) ON CONFLICT(example_id, origin_id) DO UPDATE SET suppressed=TRUE, updated_at=unixepoch()`); err != nil { return nil, err }
    if s.stmtUnsuppressState, err = db.Prepare(`UPDATE delta_form_state SET suppressed = FALSE, updated_at = unixepoch() WHERE example_id = ? AND origin_id = ?`); err != nil { return nil, err }

    if s.stmtResolveByDeltaID, err = db.Prepare(`SELECT example_id FROM delta_form_delta WHERE id = ? LIMIT 1`); err != nil { return nil, err }
    if s.stmtResolveByOrderRefID, err = db.Prepare(`SELECT example_id FROM delta_form_order WHERE ref_id = ? LIMIT 1`); err != nil { return nil, err }
    return s, nil
}
...
func (s *Service) SelectAsc(ctx context.Context, ex idwrap.IDWrap) ([]OrderRow, error) { rows, err := s.stmtSelectOrderAsc.QueryContext(ctx, ex.Bytes()); if err != nil { return nil, err }; defer rows.Close(); var out []OrderRow; for rows.Next() { var r OrderRow; if err := rows.Scan(&r.RefKind, &r.RefID, &r.Rank, &r.Revision); err != nil { return nil, err }; out = append(out, r) }; return out, rows.Err() }
