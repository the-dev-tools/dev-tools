...
1.  **Ambiguity Resolution:** Solving the "Null vs. Undefined" problem in strongly typed systems (Protobuf/Go) to distinguish between *ignoring* a field and *erasing* it.
...
The `files` table acts as the skeleton of the workspace. It cares *only* about hierarchy, ordering, and identity. It does not know *what* an HTTP request is; it only knows where it lives.
...
**Implication:**
This allows a "Draft" to be lightweight. If you only change the HTTP Method in a draft, the `delta_url`, `delta_body`, etc., remain `NULL`. The Draft strictly stores the *Difference*.

### 3.3 The "Unset" Sentinel
What if the user *deletes* the URL in a draft? We cannot simply set `delta_url = NULL`, because `NULL` means "Inherit from Parent".
...
*   **Action:** A new row is inserted into `http_header`.
    *   `http_id`: Points to the *Delta* Request.
    *   `parent_header_id`: Points to the *Original* Header.
    *   `is_delta`: TRUE.
