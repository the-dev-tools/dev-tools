...
func TestRequestNodeLoopExecution(t *testing.T) {
	...
	mockResponse := &http.Response{
		StatusCode: 200,
		Body:       io.NopCloser(strings.NewReader(`{"status":"success","data":"test"}`)),
		Header:     http.Header{"Content-Type": []string{"application/json"}},
	}
	mockClient := httpmockclient.NewMockHttpClient(mockResponse)

	// Create REQUEST node with mock client
	requestNode := New(
		requestNodeID,
		"TestRequest",
		endpoint,
...
func TestRequestNodeNestedLoopExecution(t *testing.T) {
	...
	mockResponse2 := &http.Response{
		StatusCode: 200,
		Body:       io.NopCloser(strings.NewReader(`{"status":"success","data":"nested_test"}`)),
		Header:     http.Header{"Content-Type": []string{"application/json"}},
	}
	mockClient2 := httpmockclient.NewMockHttpClient(mockResponse2)
