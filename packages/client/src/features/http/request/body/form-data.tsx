...
export const BodyFormDataTable = ({ deltaHttpId, httpId, isReadOnly = false }: BodyFormDataTableProps) => {
  ...
  return (
    <ReactTableNoMemo
      ...
      data={items}
      getRowId={(_) => collection.utils.getKey({ httpBodyFormDataId: _.httpBodyFormDataId! })}
    >
      {(table) => (
        <DataTable
          {...(!isReadOnly && addRow)}
