...
import (
	...
	"the-dev-tools/nodes/pkg/resolver"
	authv1 "the-dev-tools/services/gen/auth/v1"
	"the-dev-tools/services/gen/auth/v1/authv1connect"
	collectionv1 "the-dev-tools/services/gen/collection/v1"
	"the-dev-tools/services/gen/collection/v1/collectionv1connect"
...
func RunEmail() {
	...
	emailInviteManager, err := emailinvite.NewEmailTemplateFile("emailinvite.html", client)
	if err != nil {
		log.Fatalf("failed to create email invite manager: %v", err)
	}

	err = emailInviteManager.SendEmailInvite(context.Background(), "ege+test@dev.tools ",
